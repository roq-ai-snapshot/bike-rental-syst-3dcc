...
type ColumnType = ColumnDef<BikeInterface, unknown>;
...
export function BikeListPage(props: BikeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getBikes({
        relations: ['organization', 'rental.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BikeInterface>>(
    () => `/bikes?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/bikes/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
