...
import { BikeListPage as ShowCaseList } from 'pages/bikes';

function DashboardPage() {
  const { session } = useSession();

  const bikeStat = useSWR<PaginatedInterface<BikeInterface>>(
    () => `/bikes`,
    () => getBikes({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Bikes',
      result: bikeStat?.data?.totalCount,
      isLoading: bikeStat.isLoading,
    },
    {
      title: 'Rentals',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`bikes`} passHref legacyBehavior>
                  <Button
